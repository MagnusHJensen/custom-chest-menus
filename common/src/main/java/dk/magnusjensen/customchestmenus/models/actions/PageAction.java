...
public record PageAction(MenuActionTypeUnified type, MenuActionTypeV1 typeV1, Optional<Integer> targetPage) implements MenuAction {
    ...
    ).apply(i, idx -> new PageAction(MenuActionTypeUnified.NEXT_PAGE, MenuActionTypeV1.NEXT_PAGE, idx)));

    public static final MapCodec<PageAction> PREVIOUS_CODEC = RecordCodecBuilder.mapCodec(i -> i.group(
        Codec.INT.optionalFieldOf("target_page_index").forGetter(PageAction::targetPage)
    ).apply(i, idx -> new PageAction(MenuActionTypeUnified.PREVIOUS_PAGE, MenuActionTypeV1.PREVIOUS_PAGE, idx)));
    ...
    ).apply(i, idx -> new PageAction(MenuActionTypeUnified.JUMP_TO_PAGE, MenuActionTypeV1.JUMP_TO_PAGE, Optional.of(idx))));
}
