...
public class NeoforgeAttachmentHelper implements IAttachmentHelper {
    ...
    public <T> T getPlayerAttachment(Player player, ResourceLocation id) {
        AttachmentType<T> attachmentType = NeoforgeAttachmentRegistry.findById(id);
    ...
    public <T> void setPlayerAttachmentSync(ServerPlayer player, T attachment, ResourceLocation id) {
        ...
        if (attachment instanceof PlayerDataAttachment playerDataAttachment) {
            // Send packet
            PacketDistributor.sendToPlayer(player, new SyncAttachmentDataS2C(playerDataAttachment));
        }
    }
}
